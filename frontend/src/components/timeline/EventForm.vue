...
function onFilesChange(e: Event) {
  ...
  if (files.length > 5) {
    error.value = 'Możesz dodać maksymalnie 5 zdjęć.'
    return
  }
  error.value = ''
  photos.value = files
...
async function submit() {
  ...
  try {
    ...
    error.value = 'Nie udało się zapisać wydarzenia.'
  } finally {
...
<template>
  <div class="fixed inset-0 z-30 flex items-center justify-center bg-black/30 px-4" @click.self="emit('close')">
    <form class="w-full max-w-md space-y-4 rounded-lg bg-white p-6 shadow-xl" @submit.prevent="submit">
      <h2 class="text-lg font-medium text-neutral-900">Nowe wydarzenie</h2>

      <div>
        <label class="block text-sm text-neutral-600">Tytuł</label>
      ...
      <div>
        <label class="block text-sm text-neutral-600">Krótki opis</label>
      ...
      <div>
        <label class="block text-sm text-neutral-600">Data</label>
      ...
      <div>
        <label class="block text-sm text-neutral-600">Link (np. do folderu w chmurze)</label>
        <input v-model="externalLink" type="url" placeholder="https://…" class="mt-1 w-full rounded border border-neutral-300 px-3 py-2 text-sm focus:border-neutral-500 focus:outline-none" />
      </div>

      <div>
        <label class="block text-sm text-neutral-600">Zdjęcia (maks. 5)</label>
        <input type="file" accept="image/*" multiple class="mt-1 w-full text-sm" @change="onFilesChange" />
      </div>

      <div>
        <label class="mb-1 block text-sm text-neutral-600">Znajomi</label>
